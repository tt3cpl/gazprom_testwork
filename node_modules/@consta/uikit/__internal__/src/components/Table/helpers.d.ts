...
export declare const createSortingState: <T extends TableRow>(by: keyof T, order?: OrderType, sortFn?: (a: T[keyof T], b: T[keyof T]) => number) => SortingState<T>;
export declare const getNewSorting: <T extends TableRow>(currentSorting: SortingState<T>, newField: keyof T, sortFn?: (a: T[keyof T], b: T[keyof T]) => number) => SortingState<T>;
export declare const getMaxLevel: <T extends TableRow>(columns: Array<TableColumn<T>>) => number;
export declare const transformColumns: <T extends TableRow>(columns: Array<TableColumn<T>>, maxLevel: number) => Array<Header<T>>[];
/**
 ...
 * }
 */
export declare const useHeaderData: <T extends TableRow>(columns: Array<TableColumn<T>>) => HeaderData<T>;
/**
...
export declare const transformRows: <T extends TableRow>(rows: T[], expandedRowIds: string[], isTableExpanded?: boolean) => TableTreeRow<T>[];
export declare function getMergedArray<TYPE>(mainArr: TYPE[], mergeArr: TYPE[]): TYPE[];
export declare function calculateColSpans<T extends TableRow>(columns: TableColumn<T>[], row: TableTreeRow<T>): number[];
